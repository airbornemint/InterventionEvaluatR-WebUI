...
#' @export
evaluatr.web.server = function(input, output, session) {
  ############################################################
  ...
  observe({
    ...
    userInput(list(
      ...
      input=switch(
        input$stockDataset,
        pnas_brazil = {
          data("pnas_brazil", package="InterventionEvaluatR", envir = environment())
          pnas_brazil
        }
      )
    ))
  ...
  output$previewPlot = renderPlotly({
    periods = function() {
      if (checkNeed(input$postStart) && checkNeed(input$postDuration)) {
        ...
        c(
          geom_segment(data=df, aes_(x=~xmin, xend=~xmax, y=~y, yend=~y)),
          geom_point(data=df, aes_(x=~xmin, y=~y)),
          geom_point(data=df, aes_(x=~xmax, y=~y))
        )
      } else {
        df = data.frame(
          xmin=min(dataTime()),
        ...
        c(
          geom_segment(data=df, aes_(x=~xmin, xend=~xmax, y=~y, yend=~y), color="#FFFFFF00"),
          geom_point(data=df, aes_(x=~xmin, y=~y), color="#FFFFFF00"),
          geom_point(data=df, aes_(x=~xmax, y=~y), color="#FFFFFF00")
        )
      }
    }
    ...
    if (!is.null(dataGroup())) {
      ggplotly(ggplot(
          ...
          geom_line(aes_(x=~t, y=~y, group=~g), size=0.1) +
          periods() +
          labs(x=NULL, y=NULL) +
          theme_minimal()
      ) %>% plotlyOptions()
      ...
      ggplotly(
        ggplot(data) +
        geom_ribbon(aes_(x=~t, ymin=~ymin, ymax=~ymax), size=0.1, fill="grey75") +
        periods() +
        labs(x=NULL, y=NULL) +
        theme_minimal()
      ) %>% plotlyOptions()
      ...
      ggplotly(
        ggplot(data) +
        geom_line(aes_(x=~t, y=~y), size=0.1) +
        periods() +
        labs(x=NULL, y=NULL) +
        theme_minimal()
      ) %>% plotlyOptions()
